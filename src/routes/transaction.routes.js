...
router.post("/checkout", requireAuth, async (req, res) => {
  ...
  if (!items?.length || !address_id || !payment_method) {
    return res.status(400).json({
      ...
      error: {
        ...
        message: "items, address_id, payment_method required",
      },
    });
  }
  ...
  if (unavailable) {
    return res.status(400).json({
      ...
      error: {
        code: "VALIDATION_ERROR",
        message: `Product "${unavailable.name}" no longer available`,
      },
    });
  }
  ...
  let ordersToCreate;
  try {
    // BUG-006 FIX: throw inside .map() without try/catch causes unhandled rejection
    // Wrapped in try/catch to return a clean 400 response on product mismatch
    ordersToCreate = items.map((item) => {
    ...
    return res.status(400).json({
      success: false,
      error: { code: "VALIDATION_ERROR", message: mapErr.message },
    });
  }
